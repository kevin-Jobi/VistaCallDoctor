...
class AvailabilityUI extends StatelessWidget {
    ...
    if (state.isSubmitting) {
      return const Center(
        child: Column(
          ...
          children: [
            CircularProgressIndicator(color: Color(0xFF4F91AF), strokeWidth: 3),
            SizedBox(height: 24),
            Text(
      ...
      builder: (context, state) {
        ...
        return Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ...selectedDays
                .map((day) => _buildSelectedDayCard(context, day))
                .toList(),
          ],
        );
      },
  ...
  void _showModernTimeSlotsDialog(BuildContext context, String day) {
    ...
    List<String> selectedSlots = List.from(
      state.availability.availableTimeSlots[day] ?? [],
    );
                        ...
                        child: Column(
                          children: possibleSlots.map((slot) {
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                color: isSelected
                                    ? const Color(0xFF4F91AF).withOpacity(0.1)
                                    : Colors.transparent,
                                borderRadius: BorderRadius.circular(12),
                                border: Border.all(
                                  color: isSelected
                                      ? const Color(0xFF4F91AF)
                                      : const Color(0xFFE2E8F0),
                                  width: 1.5,
                                ),
                              ),
  ...
  void _handleContinuePressed(BuildContext context) {
    ...
    final currentState = context.read<AvailabilityBloc>().state;
    for (var day in currentState.availability.availableDays) {
      if ((currentState.availability.availableTimeSlots[day] ?? []).isEmpty) {
        _showModernSnackBar(
          context,
