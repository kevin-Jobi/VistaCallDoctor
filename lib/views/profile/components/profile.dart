...
import 'package:vista_call_doctor/views/profile/edit_field_screen.dart';
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => DoctorProfileBloc(authBloc: context.read<AuthBloc>())
        ..add(FetchProfile(null)),
      child: Scaffold(
  ...
  Widget _buildBody(BuildContext context) {
    return BlocBuilder<DoctorProfileBloc, DoctorProfileState>(
      builder: (context, state) {
        ...
        // Use state directly with safe type conversion
        final name = state.fullName ?? 'N/A';
        final experience = state.experience ?? 'N/A';
        final phone = state.phone ?? 'N/A';
        final email = state.email ?? 'N/A';
        final category = state.specialization ?? 'N/A';
        final profileImageUrl = state.profileImageUrl;
        final availability = state.availability ?? {};
        final availableDays = (availability['availableDays'] as List?)
                ?.whereType<String>()
                .toList() ??
            [];
        final availableTimeSlots = (availability['availableTimeSlots'] as Map?)?.map(
              (key, value) => MapEntry(
                key.toString(),
        ...
        final yearsOfExperience = availability['yearsOfExperience']?.toString() ?? 'N/A';
        final fees = availability['fees']?.toString() ?? 'N/A';
        ...
        final availabilityState = AvailabilityState(
          availability: Availability(
            ...
            fees: fees,
            availableTimeSlots: availableTimeSlots.cast<String, List<String>>(),
          ),
        );
  ...
  Widget _buildAvailabilitySection(
    ...
    // Use the passed availability parameter
    final initialDays = (availability['availableDays'] as List?)?.whereType<String>().toList() ?? [];
    final initialSlots = (availability['availableTimeSlots'] as Map?)?.map(
          (key, value) => MapEntry(
            key.toString(),
            (value as List?)?.whereType<String>().toList() ?? [],
          ),
        ...
        {};
    final initialYears = availability['yearsOfExperience']?.toString() ?? '';
    final initialFees = availability['fees']?.toString() ?? '';
...
//           ),
//           const SizedBox(height: 24),
//           _buildAvailabilitySection(context, availableDays, availableTimeSlots),
//           const SizedBox(height: 24),
//           _buildDocumentsSection(context),
//         ],
//       ),
...
//             // Create AvailabilityState from fetched data
//             final availabilityState = AvailabilityState(
//               availability: Availability(
//                 availableDays: availableDays,
//                 yearsOfExperience: yearsOfExperience,
...
//     List<String> availableDays,
//     Map<String, List<String>> availableTimeSlots,
//     AvailabilityState availabilityState, // Added parameter
//   ) {
//     return SingleChildScrollView(
//       padding: const EdgeInsets.fromLTRB(20, 8, 20, 20),
//       child: Column(
...
//             availableDays,
//             availableTimeSlots,
//             availabilityState, // Pass the state
//           ),
//           const SizedBox(height: 24),
//           _buildDocumentsSection(context),
//         ],
...
//     List<String> availableDays,
//     Map<String, List<String>> availableTimeSlots,
//     AvailabilityState availabilityState, // Updated parameter
//   ) {
//     final doctorProfileBloc = context.read<DoctorProfileBloc>();
//     final authBloc = context.read<AuthBloc>();
//     final user = _getCurrentUser(authBloc.state);
...
//       final availability = data['availability'] ?? {};
//       final initialDays = List<String>.from(
//         availability['availableDays'] ?? [],
//       );
...
//   User? _getCurrentUser(AuthState state){
//     if(state is Authenticated){
//       return state.user;
//     }
//     return Firebase.instance.currentUser;
//   }
...
//   // Updated to return UserModel? to match AuthState
//   UserModel? _getCurrentUser(AuthState state) {
//     if (state is Authenticated) {
//       return state.user; // Returns UserModel from Authenticated state
//     }
//     // Convert FirebaseAuth User to UserModel if needed
//     final firebaseUser = FirebaseAuth.instance.currentUser;
//     return firebaseUser != null ? UserModel.fromFirebaseUser(firebaseUser) : null;
//   }
