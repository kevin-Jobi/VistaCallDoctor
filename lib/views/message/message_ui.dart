  ...
  Widget _buildMessagesList(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (context, index) =>
          _buildMessageItem(state.messages[index], context),
    );
  }
  ...
  Widget _buildMessageItem(MessageModel message, BuildContext context) {
    return ListTile(
      leading: CircleAvatar(
        child: Text(message.senderName[0]), // Assuming a simple avatar
      ),
      title: Text(
        message.senderName,
      ...
      subtitle: Row(
        mainAxisSize: MainAxisSize.min, // Limit the row's size
        children: [
          Expanded(
          ...
          Text(
            message.time
                .split(' ')[1]
                .split('.')[0], // Show only time (e.g., "11:00")
            style: TextStyle(color: Colors.grey),
          ),
        ],
      ),
      trailing: message.messageCount > 0
          ? CircleAvatar(
              radius: 12,
